...
int MSD_APP_Read_RootCert(uint8_t *rootCert, size_t *pRootCertSize) {
    ...
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_RootCert: tng_atcacert_root_cert_size Failed \r\n");
        return status;
    }

    status = tng_atcacert_root_cert((uint8_t*) & rootCert, pRootCertSize);
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_RootCert: tng_atcacert_root_cert Failed \r\n");
        return status;
    }
    return 0;
}
...
int MSD_APP_Read_SignerCert(uint8_t *signerCert, size_t *pSignerCertSize) {
    ...
    /*Read signer cert*/
    status = tng_atcacert_max_signer_cert_size(pSignerCertSize);
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_SignerCert: tng_atcacert_max_signer_cert_size Failed \r\n");
        return status;
    }

    status = tng_atcacert_read_signer_cert((uint8_t*) & signerCert, pSignerCertSize);
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_SignerCert: tng_atcacert_read_signer_cert Failed \r\n");
        return status;
    }
...
int MSD_APP_Read_DeviceCert(uint8_t *deviceCert, size_t *pDeviceCertSize) {
    ...
    status = tng_atcacert_max_signer_cert_size(&signerCertSize);
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_DeviceCert: tng_atcacert_max_signer_cert_size Failed \r\n");
        return status;
    }
    uint8_t signerCert[signerCertSize];
    status = tng_atcacert_read_signer_cert((uint8_t*) & signerCert, &signerCertSize);
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_DeviceCert: tng_atcacert_read_signer_cert Failed \r\n");
        return status;
    }
    ...
    if (ATCA_SUCCESS != status) {
        SYS_CONSOLE_PRINT("    MSD_APP_Read_DeviceCert: tng_atcacert_read_device_cert Failed (%x) \r\n", status);
        return status;
    }
...
int MSD_APP_Read_DeviceCertPem(uint8_t *deviceCertPem, size_t *pDeviceCertPemSize) {
    ...
    MSD_APP_Read_DeviceCert((uint8_t *)&deviceCert, &deviceCertSize);
